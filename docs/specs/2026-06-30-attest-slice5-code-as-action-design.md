...
## 12. 实现期修订二：三段式 计划→执行→复盘（2026-06-30，live 验收暴露后与用户确认）

**live 暴露的两个同根问题**：
1. **冻结谎报**：Code-as-Action 的 finish 答案是模型**在生成程序时写死的**，反映不了运行时真相。实测：用户对第 2 个工单按拒绝（取消），最终答案仍说"全部已解决"，且 `computeOutcome` 因"有成功的写就吞掉被拒项"而盖 `completed`。
2. **机械感**：模型只在开头想一次就退出循环，执行交给不随机应变的解释器。

**同一病根**：模型不在循环里、叙述冻结在计划阶段。对照 Claude Code（ReAct，模型全程在循环、每步看真实结果再决定）——我们要在保住"少往返"红利的前提下找回准确与鲜活。

**修订决策：三段式（plan → execute → reflect）**
1. **plan**：模型出程序（1 次 LLM），`summarizeProgram` 推导人话计划并 yield `plan` 步。
2. **execute**：解释器带 held/verify 批量执行（0 次 LLM）；执行步实时呈现（清单逐条推进）。
3. **reflect**：把**证据账本里的真实结果**喂回模型，限定只能 `finish`，让它写**基于真相**的最终回答（1 次 LLM）。共 2 次 LLM，远少于 ping-pong 的 N 次。

**诚实底线（纯账本算、与 reflect 是否撒谎无关的 defense-in-depth）**：
- `Outcome` 增加 `'partial'`：有成功写但**也有被拒授权**时＝部分完成（不再盖干净 `completed`）。`computeOutcome`（ping-pong 用，三态）**不动**；程序路径用本地 `programOutcome`（含 partial）。
- 程序 finish 一律**追加证据小结**（"成功 N·取消 M·未验证 K"），冻结/复盘里的任何夸大都被账本数字当场戳穿。
- 空账本（这回合没干活）仍加注"未执行任何动作"（§11 修复保留）。

**不变量不变**：只引用真实 ref / 写写 verify / 高危 held / 作用域授权全照旧；reflect 只能 `finish`、不能再执行动作，且其措辞仍受证据小结+outcome 约束。

**演进接缝**：reflect 目前是"一次复盘"；§10.3 的"多程序/重规划"是它的自然延伸（复盘后若未达成，允许再出一段程序）。本次仍只跑一段程序 + 一次复盘。
