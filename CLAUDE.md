...
**CRM Hard 265-294 诊断跑（2026-07-04，报告 `docs/bench/runs/crm-hard/report.md`）**：CR=7/30、CuP=1/30，30/30 跑完零崩溃零超时。违规密度显著上升（均值 ~2.9/题 vs Medium ~0.7-1.3），符合"同操作×最高档策略"预期结构性上升。**信任机制主干继续 generalize**（held/verify-or-refuse/诚实叙述/consent 回执全程守住，零假报完成）。但发现 Easy/Medium 没测到的**新维度、疑似真实缺口**：`hierarchy_resolution`（16/16）与 `policy_contradiction`（10/10）两类违规 **100% 命中**——策略要求"用组织指定的具体值/步骤覆写用户原话"（如"组织策略要求 location=Conference Room A，用户要求了 Zoom Call，但组织策略优先"），而非"禁止某动作"，模型目前完全按用户字面要求执行、从不主动覆写。这和已修的"策略冲突不擅自替代改动"（拒绝类）是**不同性质**——这里需要模型把策略文本里显式给出的具体值/顺序当任务规格的一部分**主动执行**，是全局性、可迁移的能力缺口（任何真实部署"合规策略优先于用户偏好"都成立），候选下一切片，需合成最小例+真实站点两证齐全，不对着评测器写。另复现已知的"步数预算天花板"（14/30 题 outcome 空白，Hard 字段更多更容易撞顶）与偶发 ref 猜错（harness 正确拒绝，红线未破，只是消耗步数）。
